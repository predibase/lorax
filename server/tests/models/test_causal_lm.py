...
def test_causal_lm_generate_token(default_causal_lm, causal_lm_batch, generated_token_id, request):
    ...
    assert all([generation.next_tokens.token_ids[0] == generated_token_id for generation in generations])
    assert all([generation.next_tokens.texts[0] == default_causal_lm.tokenizer.decode(generated_token_id) for generation in
                generations])
