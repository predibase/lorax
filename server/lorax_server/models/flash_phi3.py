...
from lorax_server.models.custom_modeling.flash_phi3_modeling import (
    GATE_UP_PROJ,
    QKV_PROJ,
    FlashPhi3ForCausalLM,
    Phi3Config,
)
...
class FlashPhi3(FlashCausalLM):
    def __init__(
        ...
        if torch.cuda.is_available():
            ...
            raise NotImplementedError("FlashPhi3 is only available on GPU")
        ...
        config = Phi3Config.from_pretrained(model_id, revision=revision, trust_remote_code=trust_remote_code)
    ...
    def adapter_target_to_layer(self) -> Dict[str, Tuple[str, torch.Tensor]]:
        ...
        for i, layer in enumerate(self.model.model.layers):
            layer_weights[(i, QKV_PROJ)] = (
                f"{prefix}.{i}.self_attn.qkv_proj",
                layer.self_attn.query_key_value,
            )
            layer_weights[(i, O_PROJ)] = (f"{prefix}.{i}.self_attn.o_proj", layer.self_attn.o_proj)

            layer_weights[(i, GATE_UP_PROJ)] = (f"{prefix}.{i}.mlp.gate_up_proj", layer.mlp.gate_up_proj)
