...
def get_hqq_linear(quantize, weight, bias=None) -> HQQLinearLayer:
    if quantize == "hqq-4bit":
        quant_config = BaseQuantizeConfig(nbits=4, group_size=128, quant_zero=True, quant_scale=True, offload_meta=True, compute_dtype=torch.bfloat16)
        ...
        quant_config = BaseQuantizeConfig(nbits=2, group_size=32, quant_zero=True, quant_scale=True, offload_meta=True, compute_dtype=torch.bfloat16)
