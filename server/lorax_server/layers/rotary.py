...
class PositionRotaryEmbedding(nn.Module):
    ...
    def forward(
        ...
        if SYSTEM == "cuda":
            ...
            ipex.llm.functional.rotary_embedding(query, key, sin, cos, query.size(-1), True)
        else:
            raise ValueError(
...
class YarnPositionRotaryEmbedding(PositionRotaryEmbedding):
    def __init__(
        ...
        *,
        ...
        self.scaling_factor = scaling_factor
        self.mscale = float(
            get_mscale(self.scaling_factor, mscale) / get_mscale(self.scaling_factor, mscale_all_dim) * self.attn_factor
    ...
    def _update_cos_sin_cache(self, dtype, device, seqlen):
        ...
        if seqlen > self._seq_len_cached or self._cos_cached.device != device or self._cos_cached.dtype != dtype:
            if seqlen > self.max_position_embeddings or True:
                inv_freq_extrapolation = _create_inv_freq(self.dim, self.base, self.inv_freq.device)
