...
def get_linear(weight, bias, quantize, fan_in_fan_out=False):
    ...
    if quantize is None:
        ...
        qweight, nbits_per_codebook, num_codebooks, out_group_size, in_group_size = weight
        linear = QuantizedLinear(qweight.shape[1], qweight.shape[0], in_group_size, out_group_size, num_codebooks, nbits_per_codebook)
        with torch.no_grad():
            linear.codes.data = qweight
            if bias is not None:
