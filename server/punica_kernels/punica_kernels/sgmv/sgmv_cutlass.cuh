...
__global__ void precompute_sgmv_args(cutlass::gemm::GemmCoord* all_problems,
                                     ...
                                     int32_t* s_start, int32_t* s_end, int32_t* ranks,
                                     int d_in, int d_out,
                                     int layer_idx) {
  int i = blockIdx.x;
  int r = ranks[i];
  ...
  ptr_x[i] = x + s_start[i] * x_stride;
  ptr_y[i] = y + s_start[i] * y_stride;
  ld_x[i] = k;
  ld_w[i] = n;
  ld_y[i] = n;
}
...
bool sgmv(DType* y, DType* x, DType** w, int32_t* s_start, int32_t* s_end, int32_t* ranks,
  ...
  precompute_sgmv_args<<<num_problems, 1, 0, stream>>>(
      ...
      (cutlass_t *)x, (cutlass_t **)w, s_start, s_end, ranks, d_in, d_out, layer_idx);
